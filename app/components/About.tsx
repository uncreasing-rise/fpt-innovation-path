...
import { Lightbulb, FlaskConical, Code, TrendingUp, Star } from 'lucide-react';
        ...
        <section id="about" className="py-28 bg-slate-50">
            ...
            >
                ...
                <div className="relative">
                    ...
                    <motion.div className="space-y-16" variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.1 }}>
                ...
                <div className="mt-28 grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                    {stats.map((stat, index) => (
                        <motion.div key={index} className="bg-white border border-slate-200/80 p-8 rounded-2xl shadow-lg" variants={itemVariants}>
...
const TimelineItem = ({ step, index }: { step: typeof journeySteps[0], index: number }) => {
    ...
    return (
        <motion.div className={`md:flex items-center w-full ${isOdd ? 'md:flex-row-reverse' : ''}`} variants={itemVariants}>
            {/* Content */}
